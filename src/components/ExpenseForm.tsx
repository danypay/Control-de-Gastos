...
export default function ExpenseForm() {
    ...
    const {state,dispatch,remaininBudget} = useBudget()
    ...
    function handleSubmit(event : FormEvent<HTMLFormElement>){
        ...
        if((expense.amount - previousAmount) > remaininBudget){
            setError('Esa cantidad se sale del presupeusto')
            return
        }

        // Agregar o Actulizar el gasto
        if(state.editingId){
    ...
    <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <div className=" flex flex-col gap-2">
            ...
            <input 
                type="number"
                id="amount"
                placeholder="Añade la cantidad del gasto Ejp. 300"
                className=" bg-slate-100 p-2"
                name="amount" 
